...
void drawWindow(
    NVGcontext* vg, const char* title, float x, float y, float w, float h)
{
...
void drawSearchBox(
    NVGcontext* vg, const char* text, float x, float y, float w, float h)
{
    NVGpaint bg;
    char icon[8];
    ...
    nvgTextAlign(vg, NVG_ALIGN_CENTER | NVG_ALIGN_MIDDLE);
    nvgText(
        vg, x + h * 0.55f, y + h * 0.55f, cpToUTF8(ICON_SEARCH, icon), NULL);
    ...
    NVGcontext* vg, const char* text, float x, float y, float w, float h)
{
    ...
    bg = nvgLinearGradient(
        vg, x, y, x, y + h, nvgRGBA(255, 255, 255, 16), nvgRGBA(0, 0, 0, 16));
    nvgBeginPath(vg);
              ...
              float t)
{
    ...
    nvgBeginPath(vg);
    nvgEllipse(
        vg, lx + dx, ly + dy + ey * 0.25f * (1 - blink), br, br * blink);
    ...
    nvgBeginPath(vg);
    nvgEllipse(
        vg, rx + dx, ry + dy + ey * 0.25f * (1 - blink), br, br * blink);
    nvgFillColor(vg, nvgRGBA(32, 32, 32, 255));
    nvgFill(vg);

    gloss = nvgRadialGradient(vg,
                              lx - ex * 0.25f,
                              ly - ey * 0.5f,
                              ex * 0.1f,
                              ex * 0.75f,
...
void drawSpinner(NVGcontext* vg, float cx, float cy, float r, float t)
{
    ...
    paint = nvgLinearGradient(
        vg, ax, ay, bx, by, nvgRGBA(0, 0, 0, 0), nvgRGBA(0, 0, 0, 128));
                    ...
                    float t)
{
    ...
    for (i = 0; i < nimages; i++)
    {
        ...
        nvgBeginPath(vg);
        nvgRoundedRect(
            vg, tx + 0.5f, ty + 0.5f, thumb - 1, thumb - 1, 4 - 0.5f);
        nvgStrokeWidth(vg, 1.0f);
    ...
    // Hide fades
    fadePaint = nvgLinearGradient(vg,
                                  x,
                                  y,
                                  x,
                                  y + 6,
...
void drawColorwheel(
    NVGcontext* vg, float x, float y, float w, float h, float t)
{
    int i;
    ...
    paint = nvgRadialGradient(
        vg, ax, ay, 7, 9, nvgRGBA(0, 0, 0, 64), nvgRGBA(0, 0, 0, 0));
    nvgBeginPath(vg);
...
int loadDemoData(NVGcontext* vg, DemoData* data)
{
    ...
    data->fontEmoji =
        nvgCreateFont(vg, "emoji", "../example/NotoEmoji-Regular.ttf");
    if (data->fontEmoji == -1)
    {
...
void drawParagraph(NVGcontext* vg,
                   float x,
                   float y,
                   float width,
                   float height,
                   ...
                   float my)
{
    ...
    const char* text = "This is longer chunk of text.\n  \n  Would have used "
                       "lorem ipsum but she    was busy jumping over the "
                       "lazy dog with the fox and all the men who came to "
    ...
    while ((nrows = nvgTextBreakLines(vg, start, end, width, rows, 3)))
    {
        for (i = 0; i < nrows; i++)
        {
            ...
            int hit =
                mx > x && mx < (x + width) && my >= y && my < (y + lineh);
            ...
            if (hit)
            {
                ...
                nglyphs = nvgTextGlyphPositions(
                    vg, x, y, row->start, row->end, glyphs, 100);
                for (j = 0; j < nglyphs; j++)
                {
                    float x0 = glyphs[j].x;
                    float x1 =
                        (j + 1 < nglyphs) ? glyphs[j + 1].x : x + row->width;
                ...
                DemoData* data)
{
    ...
    drawButton(
        vg, ICON_TRASH, "Delete", x, y, 160, 28, nvgRGBA(128, 16, 8, 255));
...
static void
setAlpha(unsigned char* image, int w, int h, int stride, unsigned char a)
{
    int x, y;
    for (y = 0; y < h; y++)
    {
