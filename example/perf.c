...
void startGPUTimer(GPUtimer* timer)
{
    ...
    glBeginQuery(GL_TIME_ELAPSED,
                 timer->queries[timer->cur % GPU_QUERY_COUNT]);
    timer->cur++;
}
    ...
    while (available && timer->ret <= timer->cur)
    {
        ...
        if (available)
        {
            /*			GLuint64 timeElapsed = 0;
               ...
               GPU_QUERY_COUNT], GL_QUERY_RESULT, &timeElapsed);
                        timer->ret++;
                        if (n < maxTimes) {
...
void renderGraph(NVGcontext* vg, float x, float y, PerfGraph* fps)
{
    ...
    if (fps->style == GRAPH_RENDER_FPS)
    {
        for (i = 0; i < GRAPH_HISTORY_COUNT; i++)
        {
            float v =
                1.0f / (0.00001f +
                        fps->values[(fps->head + i) % GRAPH_HISTORY_COUNT]);
            float vx, vy;
            if (v > 80.0f)
                v = 80.0f;
    ...
    else if (fps->style == GRAPH_RENDER_PERCENT)
    {
        ...
        {
            float v =
                fps->values[(fps->head + i) % GRAPH_HISTORY_COUNT] * 1.0f;
            float vx, vy;
            if (v > 100.0f)
                v = 100.0f;
            vx = x + ((float) i / (GRAPH_HISTORY_COUNT - 1)) * w;
    ...
    else
    {
        ...
        {
            float v =
                fps->values[(fps->head + i) % GRAPH_HISTORY_COUNT] * 1000.0f;
            float vx, vy;
            if (v > 20.0f)
                v = 20.0f;
            vx = x + ((float) i / (GRAPH_HISTORY_COUNT - 1)) * w;
