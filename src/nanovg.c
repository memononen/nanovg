...
void nvgTransformPoint(
    float* dx, float* dy, const float* t, float sx, float sy)
{
...
void nvgGlobalCompositeBlendFuncSeparate(
    NVGcontext* ctx, int srcRGB, int dstRGB, int srcAlpha, int dstAlpha)
{
...
static void nvg__addPath(NVGcontext* ctx)
{
    ...
    if (ctx->cache->npaths + 1 > ctx->cache->cpaths)
    {
        ...
        paths =
            (NVGpath*) realloc(ctx->cache->paths, sizeof(NVGpath) * cpaths);
        if (paths == NULL)
            return;
...
static void nvg__addPoint(NVGcontext* ctx, float x, float y, int flags)
{
    ...
    if (ctx->cache->npoints + 1 > ctx->cache->cpoints)
    {
        ...
        points = (NVGpoint*) realloc(ctx->cache->points,
                                     sizeof(NVGpoint) * cpoints);
        if (points == NULL)
            return;
...
static NVGvertex* nvg__allocTempVerts(NVGcontext* ctx, int nverts)
{
    if (nverts > ctx->cache->cverts)
    {
        ...
        int cverts = (nverts + 0xff) & ~0xff; // Round up to prevent
                                              // allocations when things
                                              // change just slightly.
        verts = (NVGvertex*) realloc(ctx->cache->verts,
...
static void nvg__chooseBevel(int bevel,
                             NVGpoint* p0,
                             NVGpoint* p1,
                             float w,
                             float* x0,
...
static NVGvertex* nvg__roundJoin(NVGvertex* dst,
                                 NVGpoint* p0,
                                 NVGpoint* p1,
                                 float lw,
                                 float rw,
                                 ...
                                 float fringe)
{
    ...
    if (p1->flags & NVG_PT_LEFT)
    {
        ...
        nvg__chooseBevel(p1->flags & NVG_PR_INNERBEVEL,
                         p0,
                         p1,
                         lw,
                         &lx0,
...
static NVGvertex* nvg__bevelJoin(NVGvertex* dst,
                                 NVGpoint* p0,
                                 NVGpoint* p1,
                                 float lw,
                                 float rw,
...
static NVGvertex* nvg__buttCapStart(NVGvertex* dst,
                                    NVGpoint* p,
                                    float dx,
                                    float dy,
                                    float w,
...
static NVGvertex* nvg__buttCapEnd(NVGvertex* dst,
                                  NVGpoint* p,
                                  float dx,
                                  float dy,
                                  float w,
...
static NVGvertex* nvg__roundCapStart(NVGvertex* dst,
                                     NVGpoint* p,
                                     float dx,
                                     float dy,
                                     float w,
                                     ...
                                     float aa)
{
    ...
    for (i = 0; i < ncap; i++)
    {
        ...
        nvg__vset(
            dst, px - dlx * ax - dx * ay, py - dly * ax - dy * ay, 0, 1);
        dst++;
...
static NVGvertex* nvg__roundCapEnd(NVGvertex* dst,
                                   NVGpoint* p,
                                   float dx,
                                   float dy,
                                   float w,
...
static int nvg__expandStroke(
    NVGcontext* ctx, float w, int lineCap, int lineJoin, float miterLimit)
{
    NVGpathCache* cache = ctx->cache;
    NVGvertex* verts;
    NVGvertex* dst;
    ...
    for (i = 0; i < cache->npaths; i++)
    {
        ...
        for (j = s; j < e; ++j)
        {
            ...
            {
                nvg__vset(
                    dst, p1->x + (p1->dmx * w), p1->y + (p1->dmy * w), 0, 1);
                dst++;
                nvg__vset(
                    dst, p1->x - (p1->dmx * w), p1->y - (p1->dmy * w), 1, 1);
                dst++;
            }
            p0 = p1++;
        }
    ...
    NVGcontext* ctx, float x1, float y1, float x2, float y2, float radius)
{
    ...
    if (nvg__ptEquals(x0, y0, x1, y1, ctx->distTol) ||
        ...
        nvg__distPtSeg(x1, y1, x0, y0, x2, y2) <
            ctx->distTol * ctx->distTol ||
        radius < ctx->distTol)
    {
        nvgLineTo(ctx, x1, y1);
        return;
    ...
    // Calculate tangential circle to lines (x0,y0)-(x1,y1) and
    // (x1,y1)-(x2,y2).
    dx0 = x0 - x1;
    dy0 = y0 - y1;
    dx1 = x2 - x1;
...
void nvgStroke(NVGcontext* ctx)
{
    ...
    if (ctx->params.edgeAntiAlias)
        nvg__expandStroke(ctx,
                          strokeWidth * 0.5f + ctx->fringeWidth * 0.5f,
                          state->lineCap,
                          state->lineJoin,
                          state->miterLimit);
    else
                      ...
                      int maxRows)
{
    ...
    // Break the line from the end of the last word, and start new line from
                    ...
                    float* bounds)
{
    ...
    width =
        fonsTextBounds(ctx->fs, x * scale, y * scale, string, end, bounds);
    if (bounds != NULL)
    {
