...
struct NVGtextRow
{
    ...
    float minx, maxx; // Actual bounds of the row. Logical with and bounds can
                      // differ because of kerning and some parts over
                      // extending.
};
...
// for rendering.
// In addition you can upload your own image. The image loading is provided by
// stb_image.
...
// rectangle, it is useful for rendering
// drop shadows or highlights for boxes. Parameters (x,y) define the top-left
// corner of the rectangle,
// (w,h) define the size of the rectangle, r defines the corner radius, and f
// feather. Feather defines how blurry
// the border of the rectangle is. Parameter icol specifies the inner color
// and ocol the outer color of the gradient.
// The gradient is transformed by the current transform when it is passed to
// nvgFillPaint() or nvgStrokePaint().
...
// center, inr and outr specify
// the inner and outer radius of the gradient, icol specifies the start color
// and ocol the end color.
// The gradient is transformed by the current transform when it is passed to
// nvgFillPaint() or nvgStrokePaint().
...
// left-top location of the image pattern,
...
// radius is r,
// and the arc is drawn from angle a0 to a1, and swept in direction dir
// (NVG_CCW, or NVG_CW).
...
// float[4],
// if the bounding box of the text should be returned. The bounds value are
// [xmin,ymin, xmax,ymax]
...
// only the sub-string will be used.
// Measured values are returned in local coordinate space.
