...
////
//
//  Incomplete text-in-3d-api example, which draws quads properly aligned to
//  be lossless
//
#if 0
                                  ...
                                  stbtt_vertex** pvertices)
{
    ...
    *pvertices = NULL;
    ...
    if (numberOfContours > 0)
    {
        ...
        // above, shifted to the end of the array so we won't overwrite it
        // when
        ...
        for (i = 0; i < n; ++i)
        {
            ...
            if (next_move == i)
            {
                ...
                if (start_off)
                {
                    ...
                    // need to find a point on the curve
                    // where we can start, and we need to save some state for
                    // when we wraparound.
                    scx = x;
                    scy = y;
                    ...
                    else
                    {
                        ...
                        ++i; // we're using point i+1 as the starting point,
                             // so skip it
                    }
                }
                else
                                             ...
                                             void* userdata)
{
    ...
    z->x = STBTT_ifloor(STBTT_FIX * e->x0 +
                        z->dx * (start_point - e->y0)); // use z->dx so when
                                                        // we offset later
                                                        // it's by the same
                                                        // amount
    z->x -= off_x * STBTT_FIX;
                                         ...
                                         float y_top)
{
    ...
    while (e)
    {
        ...
        else
        {
            ...
            // the
            // line segment starts on this scanline. x0 is the intersection of
            // the
            ...
            else
            {
                ...
                for (x = 0; x < len; ++x)
                {
                    // cases:
                    //
                    // there can be up to two intersections with the pixel.
                    // any intersection
                    // with left or right edges can be handled by splitting
                    // into two (or three)
                    ...
                    // then used some simple logic to produce up to three
                    // segments in sorted order
                    // from top-to-bottom. however, this had a problem: if an
                    // x edge was epsilon
                    // across the x border, then the corresponding y position
                    // might not be distinct
                    // from the other y segment, and it might ignored as an
                    // empty segment. to avoid
...
static stbtt_int32
stbtt__CompareUTF8toUTF16_bigendian_prefix(const stbtt_uint8* s1,
                                           stbtt_int32 len1,
