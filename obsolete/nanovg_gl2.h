...
static struct GLNVGtexture* glnvg__allocTexture(struct GLNVGcontext* gl)
{
    ...
    if (tex == NULL)
    {
        if (gl->ntextures + 1 > gl->ctextures)
        {
            ...
            gl->textures = (struct GLNVGtexture*) realloc(
                gl->textures, sizeof(struct GLNVGtexture) * gl->ctextures);
            if (gl->textures == NULL)
                return NULL;
        }
...
static int glnvg__createShader(struct GLNVGshader* shader,
                               const char* name,
                               const char* vshader,
                               const char* fshader)
{
...
static void glnvg__getUniforms(struct GLNVGshader* shader)
{
    ...
    shader->loc[GLNVG_LOC_SCISSORMAT] =
        glGetUniformLocation(shader->prog, "scissorMat");
    shader->loc[GLNVG_LOC_SCISSOREXT] =
        glGetUniformLocation(shader->prog, "scissorExt");
    shader->loc[GLNVG_LOC_SCISSORSCALE] =
        glGetUniformLocation(shader->prog, "scissorScale");
    shader->loc[GLNVG_LOC_PAINTMAT] =
        glGetUniformLocation(shader->prog, "paintMat");
    shader->loc[GLNVG_LOC_EXTENT] =
        glGetUniformLocation(shader->prog, "extent");
    shader->loc[GLNVG_LOC_RADIUS] =
        glGetUniformLocation(shader->prog, "radius");
    shader->loc[GLNVG_LOC_FEATHER] =
        glGetUniformLocation(shader->prog, "feather");
    shader->loc[GLNVG_LOC_INNERCOL] =
        glGetUniformLocation(shader->prog, "innerCol");
    shader->loc[GLNVG_LOC_OUTERCOL] =
        glGetUniformLocation(shader->prog, "outerCol");
    shader->loc[GLNVG_LOC_STROKEMULT] =
        glGetUniformLocation(shader->prog, "strokeMult");
    shader->loc[GLNVG_LOC_TEX] = glGetUniformLocation(shader->prog, "tex");
    shader->loc[GLNVG_LOC_TEXTYPE] =
        glGetUniformLocation(shader->prog, "texType");
...
static int glnvg__renderCreate(void* uptr)
{
    ...
    static const char* fillFragShaderEdgeAA =
        ...
        "}\n"
        "\n"
        "// Stroke - from [0..1] to clipped pyramid, where the slope is "
        "1px.\n"
        "float strokeMask() {\n"
    ...
    if (gl->edgeAntiAlias)
    {
        if (glnvg__createShader(&gl->shader,
                                "shader",
                                fillVertShader,
                                fillFragShaderEdgeAA) == 0)
            return 0;
    }
    else
    {
        if (glnvg__createShader(
                &gl->shader, "shader", fillVertShader, fillFragShader) == 0)
            return 0;
    }
...
static int glnvg__renderCreateTexture(
    void* uptr, int type, int w, int h, const unsigned char* data)
{
    struct GLNVGcontext* gl = (struct GLNVGcontext*) uptr;
...
static int glnvg__renderUpdateTexture(void* uptr,
                                      int image,
                                      int x,
                                      int y,
                                      int w,
...
static int glnvg__setupPaint(struct GLNVGcontext* gl,
                             ...
                             float fringe)
{
    ...
    else
    {
        ...
        scissorsx = sqrtf(scissor->xform[0] * scissor->xform[0] +
                          scissor->xform[2] * scissor->xform[2]) /
                    fringe;
        scissorsy = sqrtf(scissor->xform[1] * scissor->xform[1] +
    ...
    if (paint->image != 0)
    {
        ...
        glUniform2f(gl->shader.loc[GLNVG_LOC_VIEWSIZE],
                    gl->viewWidth,
                    gl->viewHeight);
        glUniformMatrix3fv(
            gl->shader.loc[GLNVG_LOC_SCISSORMAT], 1, GL_FALSE, scissorMat);
        glUniform2f(gl->shader.loc[GLNVG_LOC_SCISSOREXT], scissorx, scissory);
        glUniform2f(
            gl->shader.loc[GLNVG_LOC_SCISSORSCALE], scissorsx, scissorsy);
        glUniformMatrix3fv(
            gl->shader.loc[GLNVG_LOC_PAINTMAT], 1, GL_FALSE, paintMat);
        glUniform2f(gl->shader.loc[GLNVG_LOC_EXTENT],
                    paint->extent[0],
                    paint->extent[1]);
        glUniform1f(gl->shader.loc[GLNVG_LOC_STROKEMULT],
                    (width * 0.5f + fringe * 0.5f) / fringe);
        glUniform1i(gl->shader.loc[GLNVG_LOC_TEX], 0);
        glUniform1i(gl->shader.loc[GLNVG_LOC_TEXTYPE],
...
static void
glnvg__renderViewport(void* uptr, int width, int height, int alphaBlend)
{
    struct GLNVGcontext* gl = (struct GLNVGcontext*) uptr;
    ...
    for (i = 0; i < npaths; i++)
    {
        ...
        if (path->nfill > 0)
        {
            glBufferSubData(GL_ARRAY_BUFFER,
                            n * sizeof(struct NVGvertex),
                            path->nfill * sizeof(struct NVGvertex),
                            &path->fill[0].x);
            n += path->nfill;
        }
        if (path->nstroke > 0)
        {
            glBufferSubData(GL_ARRAY_BUFFER,
                            n * sizeof(struct NVGvertex),
                            path->nstroke * sizeof(struct NVGvertex),
                            &path->stroke[0].x);
            n += path->nstroke;
        }
    }
                              ...
                              int npaths)
{
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 maxCount * sizeof(struct NVGvertex),
                 NULL,
                 GL_STREAM_DRAW);
    ...
    if (npaths == 1 && paths[0].convex)
    {
        ...
        for (i = 0; i < npaths; i++)
        {
            ...
            glVertexAttribPointer(
                ...
                GL_FALSE,
                sizeof(struct NVGvertex),
                (const GLvoid*) (offset + 2 * sizeof(float)));
    ...
    else
    {
        ...
        glVertexAttribPointer(
            0, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(float), (const GLvoid*) 0);
                                   ...
                                   int nverts)
{
    ...
    glUniform2f(
        gl->shader.loc[GLNVG_LOC_VIEWSIZE], gl->viewWidth, gl->viewHeight);
    glUniform1i(gl->shader.loc[GLNVG_LOC_TEX], 0);
    glUniform1i(gl->shader.loc[GLNVG_LOC_TEXTYPE],
                (tex != NULL && tex->type == NVG_TEXTURE_RGBA) ? 0 : 1);
    glnvg__checkError("tris solid img loc");

    glBindBuffer(GL_ARRAY_BUFFER, gl->vertBuf);
    glBufferData(GL_ARRAY_BUFFER,
                 nverts * sizeof(struct NVGvertex),
                 verts,
                 GL_STREAM_DRAW);
...
#endif
{
    ...
    struct GLNVGcontext* gl =
        (struct GLNVGcontext*) malloc(sizeof(struct GLNVGcontext));
    if (gl == NULL)
        goto error;
